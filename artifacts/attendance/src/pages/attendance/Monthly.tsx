  ...
  return (
    ...
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <path
          d="M2 3.5C2 2.67 2.67 2 3.5 2H9.5L14 6.5V12.5C14 13.33 13.33 14 12.5 14H3.5C2.67 14 2 13.33 2 12.5V3.5Z"
          fill="white"
          fillOpacity="0.15"
        />
        <path
  ...
  return (
    ...
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <path
          d="M9 2H13.5C13.78 2 14 2.22 14 2.5V13.5C14 13.78 13.78 14 13.5 14H9V2Z"
          fill="white"
          fillOpacity="0.12"
        />
        <line
...
function ManualAttendanceModal({
  ...
  // Normalize "HH:MM:SS" â†’ "HH:MM" so <input type="time"> shows the value.
  const normTime = (t?: string | null) => (t ? t.slice(0, 5) : "");
  const initialStatus = (
    ["present", "late", "half_day", "absent"].includes(cell.currentStatus)
      ? cell.currentStatus
      : "present"
  ) as "present" | "late" | "half_day" | "absent";
  ...
  const STATUS_OPTIONS = [
    {
      value: "present",
      label: "Present",
      desc: "Full day, on time",
      color: "green",
    },
    {
      value: "late",
      label: "Late",
      desc: "Arrived after grace period",
      color: "amber",
    },
    {
      value: "half_day",
      label: "Half Day",
      desc: "Partial attendance",
      color: "yellow",
    },
    {
      value: "absent",
      label: "Absent",
      desc: "No attendance recorded",
      color: "red",
    },
  ] as const;
  ...
  return (
    <div className="px-6 py-5 space-y-4">
      <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/40 rounded-lg px-3 py-2">
        ...
        <span className="ml-auto text-xs px-2 py-0.5 rounded bg-muted text-muted-foreground font-semibold">
          Manual Override
        </span>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="text-[10px] text-muted-foreground font-medium block mb-1">
                In Time 2{" "}
                <span className="text-muted-foreground/60">(2nd session)</span>
              </label>
              <input
                type="time"
                value={inTime2}
      ...
      <div>
        ...
        <textarea
          ...
          rows={2}
          placeholder="e.g. Biometric not captured, site visit..."
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        >
          {submitting ? "Savingâ€¦" : success ? "Saved!" : "Save Attendance"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/40 rounded-lg px-3 py-2">
          ...
          {cell.currentStatus === "leave" && (
            <span className="ml-auto text-xs px-2 py-0.5 rounded bg-blue-100 text-blue-700 font-semibold">
              {cell.currentLeaveType === "annual"
                ? "Annual Leave"
                : cell.currentLeaveType === "casual"
                  ? "Casual Leave"
                  : "Leave"}
            </span>
          )}
          ...
          {loadingBalance ? (
            <div className="flex gap-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="flex-1 h-16 rounded-xl bg-muted/50 animate-pulse"
                />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <p
                  className={cn(
                    "text-2xl font-bold",
                    annualOk ? "text-green-700" : "text-red-600",
                  )}
                >
                ...
                <p className="text-[10px] text-muted-foreground">
                  {balance?.annualLeaveUsed ?? 0} used /{" "}
                  {balance?.annualLeaveBalance ?? 0} total
                </p>
              </div>
              <div
                className={cn(
              ...
              >
                ...
                <p
                  className={cn(
                    "text-2xl font-bold",
                    casualOk ? "text-green-700" : "text-red-600",
                  )}
                >
                ...
                <p className="text-[10px] text-muted-foreground">
                  {balance?.casualLeaveUsed ?? 0} used /{" "}
                  {balance?.casualLeaveBalance ?? 0} total
                </p>
              </div>
            </div>
          )}
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {annualOk && (
                <span className="text-xs text-green-600 font-semibold">
                  {(balance?.annualRemaining ?? 0).toFixed(1)} days left
                </span>
              )}
            </button>
            ...
            >
              ...
              {casualOk && (
                <span className="text-xs text-green-600 font-semibold">
                  {(balance?.casualRemaining ?? 0).toFixed(1)} days left
                </span>
              )}
            </button>
        ...
        {selectedType === "no_pay" && (
          <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-orange-50 border border-orange-200">
            ...
            <p className="text-xs text-orange-700">
              This day will be marked as absent and{" "}
              <strong>1 day's salary will be deducted</strong> during payroll
              calculation.
            </p>
          </div>
        )}
      ...
      <div className="px-6 py-4 border-t border-border flex items-center gap-3 bg-muted/20">
        ...
        >
          {submitting ? "Savingâ€¦" : success ? "Saved!" : "Confirm Leave"}
        </button>
      </div>
    </>
  );
...
export default function MonthlySheet() {
  ...
  const handleExportPdf = () => {
    const dayHeader = daysArray
      .map(
        (d) =>
          `<th style="min-width:28px;text-align:center;padding:4px 2px;font-size:8px;background:#1565a8;color:#fff;">${d}<br/><span style="font-size:7px;opacity:.8">${getDayName(d)}</span></th>`,
      )
      .join("");
    const bodyRows = rows
      .map(
        ...
        <td style="text-align:center;font-family:monospace;color:#ea580c;padding:4px;border-bottom:1px solid #f0f0f0">${row.overtimeHours > 0 ? fmtHrs(row.overtimeHours) : "â€”"}</td>
      </tr>`,
      )
      .join("");
    ...
    w.document
      .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Monthly Sheet â€“ ${monthName} ${year}</title>
    ...
    <div style="padding:12px 20px 8px;border-bottom:3px solid #1565a8;background:#f5f8ff;display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="font-size:10px;color:#6b7280">${monthName} ${year} Â· ${rows.length} Employees</div></div>
  ...
  const handleExportExcel = () => {
    ...
    const csvRows = rows.map((row: any) => {
      ...
      return [
        ...
        row.absentDays ?? 0,
        row.lateDays ?? 0,
        row.totalWorkHours != null ? row.totalWorkHours.toFixed(1) : "",
        row.overtimeHours > 0 ? row.overtimeHours.toFixed(1) : "0",
      ];
    });
    ...
    a.href = url;
    a.download = `monthly-sheet-${monthName}-${year}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-4">
        ...
        <Card className="overflow-hidden">
          <div className="w-full overflow-auto max-h-[calc(100vh-260px)]">
            <table className="text-xs border-collapse min-w-max">
              <thead>
                <tr>
                  <th className="px-4 py-2.5 bg-muted font-semibold border-b border-r border-border sticky left-0 top-0 z-40 min-w-[220px] text-left shadow-[1px_0_0_0_hsl(var(--border))]">
                    Employee
                  </th>
                  {daysArray.map((day) => (
                    <th
                      key={day}
                      className={cn(
                        "px-1 py-1 font-semibold border-b border-border text-center sticky top-0 z-30",
                        showTimes ? "min-w-[68px]" : "min-w-[34px]",
                        isSunday(day) ? "bg-red-50 text-red-600" : "bg-muted",
                      )}
                    >
                  ...
                  <th className="px-2 py-2.5 bg-green-50 text-green-700 font-bold border-b border-border text-center min-w-[36px] sticky top-0 z-30">
                    P
                  </th>
                  <th className="px-2 py-2.5 bg-red-50 text-red-600 font-bold border-b border-border text-center min-w-[36px] sticky top-0 z-30">
                    A
                  </th>
                  <th className="px-2 py-2.5 bg-amber-50 text-amber-600 font-bold border-b border-border text-center min-w-[36px] sticky top-0 z-30">
                    L
                  </th>
                  <th className="px-3 py-2.5 bg-blue-50 text-blue-700 font-bold border-b border-border text-center min-w-[60px] sticky top-0 z-30">
                    Total Hrs
                  </th>
                  <th className="px-3 py-2.5 bg-orange-50 text-orange-700 font-bold border-b border-border text-center min-w-[56px] sticky top-0 z-30">
                    OT Hrs
                  </th>
                  <th className="px-3 py-2.5 bg-indigo-50 text-indigo-700 font-bold border-b border-border text-center min-w-[80px] sticky top-0 z-30">
                    Reports
                  </th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-3 py-2 bg-card border-r border-border sticky left-0 z-10 shadow-[1px_0_0_0_hsl(var(--border))] group-hover:bg-muted/10">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        {row.employeeCode} Â· {row.designation}
                      </div>
                    </td>
                    {daysArray.map((day) => {
                      const entry = row.dailyStatus?.find(
                        (d: any) => d.day === day,
                      );
                      const st = entry?.status || "absent";
                      const effectiveSt =
                        st === "absent" && isSunday(day) ? "sunday" : st;
                      ...
                      const clickable =
                        !isSunday(day) && effectiveSt !== "holiday";
                      ...
                      return (
                        ...
                        >
                          {showTimes ? (
                            <div
                              onClick={() =>
                                clickable && handleCellClick(row, day)
                              }
                              title={
                                clickable
                                  ? "Click to set attendance or leave"
                                  : undefined
                              }
                              className={cn(
                                ...
                                cfg.bg,
                                clickable &&
                                  "cursor-pointer hover:ring-2 hover:ring-indigo-400 hover:ring-offset-1 transition-all",
                              )}
                            >
                              <span
                                className={cn(
                              ...
                              {entry?.leaveType && effectiveSt === "leave" && (
                                <span className="text-[7px] text-blue-600 font-semibold leading-tight uppercase">
                                  {entry.leaveType}
                                </span>
                              )}
                              {inT && (
                                <span className="text-[8px] leading-tight text-blue-600 font-mono">
                                  {inT}
                                </span>
                              )}
                              {outT && (
                                <span className="text-[8px] leading-tight text-blue-400 font-mono">
                                  {outT}
                                </span>
                              )}
                              {inT2 && (
                                <span className="text-[8px] leading-tight text-orange-600 font-mono">
                                  {inT2}
                                </span>
                              )}
                              {outT2 && (
                                <span className="text-[8px] leading-tight text-orange-400 font-mono">
                                  {outT2}
                                </span>
                              )}
                              {hrs != null && (
                                <span className="text-[8px] leading-tight font-semibold text-gray-700">
                                  {fmtHrs(hrs)}
                                </span>
                              )}
                            </div>
                    ...
                    <td className="px-2 py-2 text-center font-bold text-green-700 bg-green-50/40">
                      {row.presentDays ?? 0}
                    </td>
                    <td className="px-2 py-2 text-center font-bold text-red-600 bg-red-50/40">
                      {row.absentDays ?? 0}
                    </td>
                    <td className="px-2 py-2 text-center font-bold text-amber-600 bg-amber-50/40">
                      {row.lateDays ?? 0}
                    </td>
                    <td className="px-3 py-2 text-center font-mono font-semibold text-blue-700 bg-blue-50/30">
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground bg-card p-3 rounded-xl border border-border">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-red-100 border border-red-200 rounded inline-block" />{" "}
          A = Absent
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-amber-100 border border-amber-200 rounded inline-block" />{" "}
          L = Late
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-yellow-100 border border-yellow-200 rounded inline-block" />{" "}
          HD = Half Day
        </span>
        <span className="flex items-center gap-1.5">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-gray-200 border border-gray-300 rounded inline-block" />{" "}
          HL = Holiday
        </span>
        <span className="ml-auto text-muted-foreground/70 flex items-center gap-1">
          <span className="inline-block w-2 h-2 rounded-full bg-blue-400" />{" "}
          Click any cell to assign leave
        </span>
      </div>
    </div>
  );
