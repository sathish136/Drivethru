...
import { Plus, Edit2, Trash2, ShieldCheck, Building2, X, Check } from "lucide-react";
...
export default function Users() {
  ...
  function openEdit(u: any) {
    setForm({ username: u.username, fullName: u.fullName, email: u.email, password: "",
              role: u.role, branchIds: u.branchIds, isActive: u.isActive });
  ...
  function handleSave() {
    if (editId) {
      const payload: any = { fullName: form.fullName, email: form.email, role: form.role,
                              branchIds: form.branchIds, isActive: form.isActive };
  ...
  function handleDelete(id: number) {
    remove.mutate({ id }, { onSuccess: () => { setConfirmId(null); refetch(); } });
  }

  function toggleBranch(id: number) {
    setForm(f => ({ ...f, branchIds: f.branchIds.includes(id)
      ? f.branchIds.filter(b => b !== id) : [...f.branchIds, id] }));
  }
        ...
        <Card className="p-5 border-primary/30 bg-primary/5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input type="email" placeholder="user@org.com" value={form.email}
                onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
            </div>
            <div>
              <Label className="text-xs">{editId ? "New Password (leave blank to keep)" : "Password"}</Label>
              <Input type="password" placeholder={editId ? "Leave blank to keep" : "Set password"}
            ...
            <div className="flex items-center gap-2 pt-5">
              <input type="checkbox" checked={form.isActive}
                ...
                className="accent-primary" id="userActive" />
                ...
                {(branches || []).map(b => (
                  <label key={b.id} className={cn(
                    ...
                    <input type="checkbox" checked={form.branchIds.includes(b.id)}
                      onChange={() => toggleBranch(b.id)} className="accent-primary" />
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-border">
                {(users || []).map((u: any) => {
                  ...
                  return (
                    <tr key={u.id} className={cn("transition-colors", isConfirming ? "bg-red-50" : "hover:bg-muted/30")}>
                      ...
                      <td className="px-3 py-2.5">
                        {u.role === "super_admin" ? (
                          <span className="flex items-center gap-1 text-muted-foreground">
                            <ShieldCheck className="w-3 h-3" /> All Branches
                          </span>
                        ) : u.branchIds?.length ? (
                          <div className="flex flex-wrap gap-1 max-w-[200px]">
                            ...
                            {u.branchIds.length > 3 && (
                              <span className="text-muted-foreground">+{u.branchIds.length - 3} more</span>
                            )}
                          </div>
                        ...
                        {isConfirming ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 rounded bg-red-600 hover:bg-red-700 text-white text-xs font-medium disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 rounded bg-muted hover:bg-muted/80 text-xs font-medium"
                            >
