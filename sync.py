...
API_URL = os.environ.get(
    "API_URL",
    "http://localhost:18763"           # change to your deployed API URL
)
PUSH_DB_PATH = os.environ.get(
    "PUSH_DB_PATH",
    "/home/erp/drive/erp-sync/push.db"
)
POLL_INTERVAL = int(os.environ.get("POLL_INTERVAL", "10"))   # seconds
BATCH_SIZE = int(os.environ.get("BATCH_SIZE", "500"))        # records per batch
    ...
    while True:
        try:
            ...
            if new_rows:
                ...
                count = len(new_rows)
                log.info(f"Found {count} new punch(es) — processing in batches of {BATCH_SIZE}...")
                ...
                for i in range(0, len(new_rows), BATCH_SIZE):
                    ...
                    try:
                        ...
                        log.info(
                            ...
                            f"({total_processed}/{count} total)"
                        )
                    finally:
                        os.unlink(tmp_db)
                
                # Only advance the pointer on success
                save_last_synced(latest_ts)
                last_synced = latest_ts
                log.info(f"All batches completed successfully — {total_processed} records synced")
            else:
